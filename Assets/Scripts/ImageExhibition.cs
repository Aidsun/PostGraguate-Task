// 文件：ImageExhibition.cs
// 模块：交互对象 / 图文展品
// 说明：该脚本挂载在主馆场景中的图文展品对象上，负责展品的数据配置、外观初始化（显示封面图）、
//      高亮反馈以及交互启动。当玩家与展品交互时，它会保存当前玩家状态到GameData的保险箱，
//      将展品数据打包存入GameData.CurrentImage，然后跳转到图文展示场景（ImageContent）。
...
{
    [Header("数据配置")]    // 在Inspector中分组显示
    public string Title;                     // 展品标题
    public Sprite ImageContent;               // 展品的封面图片（用于在3D物体上显示）
    [TextArea] public string Description;     // 展品的描述文字（支持多行文本）
